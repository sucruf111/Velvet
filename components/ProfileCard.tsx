...
import { Phone, MapPin, CheckCircle2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const ProfileCard: React.FC<ProfileCardProps> = ({ profile }) => {
  ...
  return (
    <div className="group relative w-full bg-neutral-900 border border-white/5 hover:border-luxury-gold/50 transition-all duration-500 hover:shadow-[0_0_20px_rgba(212,175,55,0.1)] overflow-hidden rounded-sm">
      ...
      {/* Image Container */}
      <Link to={`/profile/${profile.id}`} className="block relative aspect-[3/4] overflow-hidden bg-neutral-950">
        <img 
        ...
        {/* Available Status Dot - Top Right */}
        {isAvailableNow && (
            <div className="absolute top-3 right-3 z-30 pointer-events-none">
                <div className="inline-flex items-center gap-1.5 bg-black/60 border border-green-500/30 rounded px-2 py-1 backdrop-blur-md shadow-lg">
                    ...
                    <span className="text-[9px] font-bold text-white uppercase tracking-wider leading-none">{t('card.available')}</span>
                </div>
            </div>
        )}
        ...
        {/* Price Tag (Bottom Right above info) */}
